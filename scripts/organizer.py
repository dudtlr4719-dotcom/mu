...
def _judge_with_claude(client, candidate, known_names):
    ...
    prompt = (
        ...
        "Decide: is this post specifically about a named celebrity, idol, or "
        "influencer wearing/using a MUUT product (sunglasses/eyewear)? Ordinary "
        "customer shopping reviews, popup-store visit posts, or general brand "
        "coverage with no named individual do NOT count — only real named-person "
        "sightings. If yes, extract the celebrity's name (Korean, as commonly "
        "written) and the product name/model if mentioned (otherwise null)."
    )
